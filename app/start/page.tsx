...
function StartContent() {
  ...
  // Half-day quiz gating: which step's "quick check" panel is currently open.
  ...
  // Half-day steps are quiz-gated: ticking opens the quiz instead of marking done.
  // (Already-done steps untick directly.) The step is only marked done once the
  // quiz is passed — see passQuiz.
  function tickHalfdayStep(id: string) {
    if (steps[id]) { const next = { ...steps, [id]: false }; setSteps(next); persistSteps(next, phone); return }
    setQuizOpen(id)
  }
  function passQuiz(id: string) {
    if (!phone && !previewUnlock) { setPendingStep(id); setPhoneAsked(true); return }
    const next = { ...steps, [id]: true }
    setSteps(next); persistSteps(next, phone); setQuizOpen(null)
  }
  ...
  function submitPhone() {
    ...
    setPendingStep(null); setPhoneInput(''); setQuizOpen(null)
  }
    ...
    <div className="relative min-h-screen text-white" style={{ background: '#060606' }}>
      ...
      <div className="max-w-lg mx-auto px-5 pb-28">
        ...
        <div className="mt-4 space-y-3">
          {/* Step 1 */}
          <StepCard n="1" done={steps['1']} onToggle={() => tickHalfdayStep('1')} locked={isLocked('1')} active={quizOpen === '1'}
            ...
            {quizOpen === '1' && <StepQuiz quiz={HALFDAY_PREP_QUIZ['1']} onPass={() => passQuiz('1')} />}
          </StepCard>

          {/* Step 2 */}
          <StepCard n="2" done={steps['2']} onToggle={() => tickHalfdayStep('2')} locked={isLocked('2')} active={quizOpen === '2'}
            ...
            {quizOpen === '2' && <StepQuiz quiz={HALFDAY_PREP_QUIZ['2']} onPass={() => passQuiz('2')} />}
          </StepCard>

          {/* Step 3 — CRITICAL, OS-aware */}
          <StepCard n="3" done={steps['3']} onToggle={() => tickHalfdayStep('3')} locked={isLocked('3')} active={quizOpen === '3'} critical
            ...
            {quizOpen === '3' && <StepQuiz quiz={HALFDAY_PREP_QUIZ['3']} onPass={() => passQuiz('3')} />}
          </StepCard>

          {/* Step 4 — NEW: connect apps via MCP / Connectors */}
          <StepCard n="4" done={steps['mcp']} onToggle={() => tickHalfdayStep('mcp')} locked={isLocked('mcp')} active={quizOpen === 'mcp'}
            ...
            {quizOpen === 'mcp' && <StepQuiz quiz={HALFDAY_PREP_QUIZ['mcp']} onPass={() => passQuiz('mcp')} />}
          </StepCard>

          {/* Step 5 — NEW: Claude for Chrome + login (autoplay video) */}
          <StepCard n="5" done={steps['chrome']} onToggle={() => tickHalfdayStep('chrome')} locked={isLocked('chrome')} active={quizOpen === 'chrome'}
            ...
            {quizOpen === 'chrome' && <StepQuiz quiz={HALFDAY_PREP_QUIZ['chrome']} onPass={() => passQuiz('chrome')} />}
          </StepCard>

          {/* Step 6 — pre-event survey (stable key '4') */}
          <StepCard n="6" done={steps['4']} onToggle={() => tickHalfdayStep('4')} locked={isLocked('4')} active={quizOpen === '4'}
            ...
            {quizOpen === '4' && <StepQuiz quiz={HALFDAY_PREP_QUIZ['4']} onPass={() => passQuiz('4')} />}
          </StepCard>

          {/* Step 7 — bring your data (stable key '5') */}
          <StepCard n="7" done={steps['5']} onToggle={() => tickHalfdayStep('5')} locked={isLocked('5')} active={quizOpen === '5'}
            ...
            {quizOpen === '5' && <StepQuiz quiz={HALFDAY_PREP_QUIZ['5']} onPass={() => passQuiz('5')} />}
          </StepCard>

          {/* Step 8 — show up early + venue (stable key '6') */}
          <StepCard n="8" done={steps['6']} onToggle={() => tickHalfdayStep('6')} locked={isLocked('6')} active={quizOpen === '6'}
            ...
            {quizOpen === '6' && <StepQuiz quiz={HALFDAY_PREP_QUIZ['6']} onPass={() => passQuiz('6')} />}
          </StepCard>
        </div>
        )}
...
function StepCard({ n, title, subtitle, done, onToggle, critical, locked, active, children }: {
  n: string; title: string; subtitle: string; done: boolean; onToggle: () => void; critical?: boolean; locked?: boolean; active?: boolean; children: React.ReactNode
}) {
  return (
    <Glass className={`p-4 transition-all ${locked ? 'opacity-50' : ''} ${done && !locked ? 'border-amber-500/30' : active && !locked ? 'border-amber-500/40' : critical && !locked ? 'border-red-500/25' : ''}`}
...
function StepQuiz({ quiz, onPass }: { quiz: PrepQuiz; onPass: () => void }) {
  ...
  return (
    <div className="mt-3 rounded-2xl p-4 border border-amber-500/25" style={{ background: 'rgba(245,158,11,0.05)' }}>
      <div className="flex items-center gap-2 mb-1.5">
        <span className="text-[10px] font-bold tracking-wider uppercase text-amber-400/90">Quick check</span>
        <span className="text-[10px] text-zinc-500">{quiz.type === 'tf' ? 'True or false' : 'Pick one'}</span>
      </div>
      <p className="text-[14px] font-semibold text-zinc-100 leading-snug mb-3">{quiz.question}</p>
        ...
        {quiz.options.map((opt, i) => {
          ...
          return (
            <button key={i} onClick={() => setPicked(i)}
              className={`w-full text-left rounded-xl px-3.5 py-2.5 text-[13px] border transition-all flex items-center gap-2.5
              ...
              <span className={`w-5 h-5 shrink-0 rounded-full border flex items-center justify-center text-[11px] font-bold
                ${showAsCorrect ? 'border-emerald-400 text-emerald-300' : showAsWrong ? 'border-red-400 text-red-300' : 'border-zinc-600 text-zinc-500'}`}>
      ...
      <div className="flex items-center gap-4 mt-3">
        ...
        {correctPicked && (
          <button onClick={onPass} className="ml-auto text-black font-semibold rounded-xl px-4 py-2 text-[13px]"
            style={{ background: 'linear-gradient(135deg, #f59e0b, #D4684A)' }}>
            Correct — continue →
          </button>
        )}
